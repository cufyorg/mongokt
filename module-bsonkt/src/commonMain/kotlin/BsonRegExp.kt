 ...
 * @since 2.0.0
 */
data class BsonRegExp(val pattern: String, val options: Set<Char> = emptySet()) : BsonElement {
 ...
 * @since 2.0.0
 */
fun BsonRegExp(pattern: String, options: String): BsonRegExp {
    val charSet = options.asSequence().sorted().toSet()
    return BsonRegExp(pattern, charSet)
}
